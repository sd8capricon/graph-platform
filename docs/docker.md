...
All Docker-related files live in `docker/`. The Compose stack runs the ASP.NET API, the nginx-served SPA, a self-built PostgreSQL 18 with Apache AGE and pgvector, RabbitMQ, the Celery worker and Beat, plus a one-shot Python schema initializer. PostgreSQL is provided by the Compose `postgres` service and is shared by all services.
...
| `docker/postgres/Dockerfile`         | `postgres:18.6-trixie` build → `postgres:18.6-trixie` runtime                              | Compiles Apache AGE 1.8.0 (PG18) and pgvector 0.8.6 from their official sources into a staging `DESTDIR`; runtime copies only the compiled libraries/SQL and the `init/` scripts. See `docker/README.md` |

### `.dockerignore`

One file per Dockerfile (`docker/Dockerfile.<name>.dockerignore`, and `docker/postgres/Dockerfile.dockerignore` for the `postgres` image). All five are currently identical:
...
| -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `PGHOST` (default `postgres`), `PGPORT` (default `5432`), `PGDATABASE`, `PGUSER`, `PGPASSWORD`                                              | `postgres`, `python-schema-init`, `ingestion-worker`, `ingestion-beat` | Credentials for the Compose PostgreSQL 18 + AGE + pgvector service; also fed to the service's `POSTGRES_*` variables                 |
| `POSTGRES_HOST_PORT` (default `5432`)                                                                                                        | `postgres` (host port mapping)                             | Host-only port the database is published on (`127.0.0.1`); containers always use `postgres:5432`                          |
| `API_PG_CONNECTION_STRING`                                                                                                                   | `api` (`ConnectionStrings__PgConnectionString`)            | Same database as `PG*` above, Npgsql format; host is the Compose service name `postgres`                                    |
...
| `127.0.0.1:${POSTGRES_HOST_PORT:-5432}:5432` | `postgres` (host access for migrations/dev tools) |
| `5087:5087`                | `api`                             |
| `8080:80`                  | `frontend`                        |
...
| -------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `../data/volumes/postgres:/var/lib/postgresql`   | PostgreSQL 18 data (bind mount). PG18's `PGDATA` is `/var/lib/postgresql/18/docker`, so the mount is the parent directory, not `/data` |
...
No custom networks are defined; all services use the Compose default network. The API, schema initializer, worker and Beat reach PostgreSQL at hostname `postgres:5432`; the worker and Beat reach RabbitMQ at hostname `rabbitmq:5672`.
...
| -------------------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `postgres`           | Built from `docker/postgres/Dockerfile`                                                               | PostgreSQL 18.6 + Apache AGE 1.8.0 + pgvector 0.8.6; bind mount `data/volumes/postgres`; `pg_isready` healthcheck; published on `127.0.0.1:${POSTGRES_HOST_PORT}`  |
...
- `api` and `python-schema-init` wait for `postgres: service_healthy`.
...
- EF migrations are never run by Compose. Start `postgres`, then apply them from the host (the host reaches the container on `localhost:${POSTGRES_HOST_PORT}`):
...
Prerequisites: Docker with Compose, .NET 10 SDK (for the one-time migration), and a filled-in `docker/.env`.

```sh
# 1. Configure
cp docker/.env.example docker/.env
# edit docker/.env: PG*, POSTGRES_HOST_PORT, API_PG_CONNECTION_STRING (same DB), JWT_SIGNING_KEY, RabbitMQ, provider keys

# 2. Start PostgreSQL, then apply API migrations (host, once per migration)
docker compose --env-file docker/.env -f docker/compose.yaml up -d postgres
set -a; . docker/.env; set +a
ConnectionStrings__PgConnectionString="Host=localhost;Port=${POSTGRES_HOST_PORT:-5432};Database=${PGDATABASE};Username=${PGUSER};Password=${PGPASSWORD}" \
  dotnet ef database update --project src/api/GraphPlatform.Api
...
| Start only PostgreSQL            | `docker compose --env-file docker/.env -f docker/compose.yaml up -d postgres`               |
| Verify AGE and pgvector          | `docker compose --env-file docker/.env -f docker/compose.yaml exec postgres psql -U "$PGUSER" -d "$PGDATABASE" -c "SELECT extname, extversion FROM pg_extension WHERE extname IN ('age','vector');"` |
