...
/// still being validated to parse as a UUID when supplied. The same split is kept here —
/// <see cref="Id"/> is a string column, and the request DTO validates UUID-ness
/// (<c>CreateModelRequest.ValidateId</c>) — because it is stamped as embedding provenance
/// (<c>embedding_model_id</c> on the Python side) and must stay stable across restarts once assigned.
/// Unlike the Python schema, the API generates one when the caller omits it (<c>ModelsController
/// .CreateModel</c>), the same convention <c>KnowledgeBase.Id</c> follows.
/// </description></item>
/// </list>
/// </remarks>
public class ModelConfig
{
    ...
    /// <summary>
    /// UUID string identifying this configured model entry, generated by the API when the caller
    /// omits one on create. It is stamped as embedding provenance and is what the Python side's
    /// <c>vector_search()</c> filters on, so it must stay stable across restarts once assigned.
    /// </summary>
