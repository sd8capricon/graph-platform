...
so the upsert logic lives in the worker. The read/vector-search methods stay on
the ORM models in `common`. The logic itself - including the two
`GraphSchemaRegistry`/`SchemaEmbedding` traps - is preserved from the previous
`GraphSchemaRegistry.upsert_records()` / `NodeEmbedding.upsert_records()`.
"""
...
def schema_embedding_text(record: GraphSchemaRegistry) -> str:
...
def node_embedding_text(record: NodeEmbedding) -> str:
...
async def upsert_schema_registry(
    ...
    for record in records:
        ...
        if existing is None:
            ...
            # iteration's `select()` autoflushes it, and reading
            # `record.embedding_row` after that raises MissingGreenlet.
    ...
    embeddings = await EmbeddingService.compute_embeddings(
        model, [schema_embedding_text(record) for record in persisted]
    )
...
async def upsert_node_embeddings(
    ...
    embeddings = await EmbeddingService.compute_embeddings(
        model, [node_embedding_text(record) for record in persisted]
    )
    if embeddings is not None:
        embedding_model_id = model.id if model is not None else None
        for record, embedding in zip(persisted, embeddings):
