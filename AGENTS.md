...
- Connection setup gotcha: every new Postgres session must run `LOAD 'age';` and `SET search_path = ag_catalog, "$user", public;` before any `ag_catalog.*` call, even fully-qualified ones like `ag_catalog.create_graph(...)` — AGE's internal DDL (e.g. the btree index it creates using its own `graphid_ops` operator class) resolves operator classes via `search_path`, not via schema-qualification of the calling query. Skipping this raises `psycopg.errors.UndefinedObject: operator class "graphid_ops" does not exist for access method "btree"` on `create_graph()`. `create_connection()` in `__init__.py` runs both statements right after connecting, before the connection is handed to `AgeGraphRepository`
