...
3. **Agents** (`src/graphrag_apacheage/agent/`)
   ...
   - `models.py` - small tool-facing pydantic models that don't belong on `AgentContext` and aren't
     ORM/`schemas/` types either. Currently just `NodeRef` (`id: str`, `label: str`): a reference to a
     node that already exists in the graph, used as `get_node_schema`'s `node` parameter instead of
     `schemas.knowledge_base.KnowledgeNode`. `KnowledgeNode.id` is optional and auto-generated by
     `KnowledgeNode.ensure_id()` when omitted — the right behavior for *authoring* a new node, but
     wrong for a *lookup* tool: an agent that forgot to pass `id` would get back a fabricated UUID
     that matches nothing in the graph instead of a validation error. `NodeRef.id` has no default, so
     an omitted `id` fails schema validation before the tool ever runs, and `get_node_schema` itself
     still rejects an empty/whitespace `id` (`not node.id or not node.id.strip()`) as defense in
     depth. `get_node_neighbours` still takes a `KnowledgeNode` (see below) — promote it to `NodeRef`
     too if the same silent-fabricated-id failure mode is ever hit there
   ...
   - `get_node_schema(node, runtime)` enriches `AgeGraphRepository.get_node_schema()`'s
     ...
     Repository Pattern") stays untouched. The tool's `node` argument is a `NodeRef` (see `models.py`
     above), which has no `properties` field at all — the agent typically calls this tool precisely
     because it doesn't know the node's real property values yet, so there is nothing to read off the
     input for this. Three separate `get_properties_by_name` calls are made
...
- Key tests:
  ...
  - `test_get_node_schema_raises_on_empty_id()` (`tests/test_tools.py`) - a `NodeRef` built with `""`
    or `"   "` as `id` still raises, the same defense-in-depth as the other tools' fail-fast checks
  - `test_node_ref_requires_id()` (`tests/test_tools.py`) - `NodeRef(label=...)` with no `id` raises
    `pydantic.ValidationError` at construction, confirming `id` is not auto-generated the way
    `KnowledgeNode.id` is
